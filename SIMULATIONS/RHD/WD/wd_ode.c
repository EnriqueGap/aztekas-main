#include "main.h"
/*
float  interior_ode(float x, float y[], int i){
    if (i==RHO) return 0;                               //no ode for rho
    if (i==PRE) return(-y[RHO]*G_cgs*y[MB]/pow(x,2));	//dP/dr = - rho*G*m/r^2
    if (i==MB) 	return(4*PI*y[RHO]*pow(x,2));	        //dm/dr = 4pi*rho*r^2
}

float  taylor_ode(float x, float y[], int i){
    if (i==RHO) return 0;                                       //no ode for rho
    if (i==PRE) return(-y[RHO]*G_cgs*4*PI*massDensity*x/3);	//dP/dr ~ - rho*G*4pi*rho_c*r/3
    if (i==MB) 	return(4*PI*y[RHO]*pow(x,2));	                //dm/dr = 4pi*rho*r^2
}
*/
double  interior_ode(float x, double y[], int i){
    
    if (i==RHO) return 0;                                           //no ode for E
    if (i==PRE){
        double numerator, denominator, returnValue;
        numerator = (4*PI*y[PRE]*pow(x,3)/pow(c_cgs,2)) + y[MB];
        denominator = x*((pow(c_cgs,2)*x/G_cgs) -2*y[MB]);
        returnValue = -(y[RHO]+y[PRE])*numerator/denominator;
        return returnValue;                                         //dP/dr = -(E+P)(4pi*P*r^3/c^2 + m)/r(c^2*r/G -2m)
    }
    if (i==MB) 	return(4*PI*y[RHO]*pow(x,2)/pow(c_cgs,2));	        //dm/dr = 4pi*E*r^2/c^2
}

double  taylor_ode(float x, double y[], int i){

    if (i==RHO) return 0;                                           //no ode for rho
    if (i==PRE){
        double numerator, denominator, central_density, returnValue;
        central_density = eos(massDensity,RHOB);
        numerator = 4*PI*(3*y[PRE]*x + central_density*x);
        denominator = 3*pow(c_cgs,4)/G_cgs - (8*PI*central_density*pow(x,2));
        returnValue= -(y[RHO]+y[PRE])*numerator/denominator;
        return returnValue;                                                 //dP/dr = -(E+P)(4pi)(3P*r + E_c*r)/3c^2*(c^2/G -8pi*E_c*r^2/3c^2))
        }
    if (i==MB) return(4*PI*y[RHO]*pow(x,2)/pow(c_cgs,2));           //dm/dr = 4pi*rho*r^2/c^2
}